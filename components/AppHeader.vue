<template>
  <header
    ...
    :class="{
      'bg-[url(~/assets/images/hero-bg.png)] bg-no-repeat bg-cover bg-center max-lg:!py-3 !py-4':
        dark,
    }"
  >
    <div class="container flex h-16 items-center">
      <div class="flex items-center max-xl:space-x-4 space-x-8">
        <NuxtLink :to="localePath('/')">
          <img
            ...
            alt=""
            class="h-16 max-lg:w-16 w-[80px] object-contain"
          />
        </NuxtLink>
        <nav
          class="hidden lg:flex items-center max-xl:space-x-4 space-x-8 max-xl:text-sm text-xl font-normal"
        >
          <button
            @click.prevent="scrollToSection('hero')"
            class="transition-300 text-white"
          >
          ...
          <button
            @click.prevent="scrollToSection('suggestions')"
            class="transition-300 text-white"
          >
      ...
      <div class="flex gap-3 justify-end flex-1">
        <DropdownMenu v-model:open="languageDropdown">
          <DropdownMenuTrigger>
            <div
              class="flex items-center justify-center gap-1 sm:gap-2 bg-white/[0.16] border-white/[0.08] px-2 py-3 rounded-lg shrink-0 text-white"
            >
              <img
                src="/assets/svg/flag/uz.svg"
                alt=""
                class="w-5 h-5"
                v-if="locale === 'uz'"
              />
              <img
                src="/assets/svg/flag/ru.svg"
                alt=""
                class="w-5 h-5"
                v-else-if="locale === 'ru'"
              />
              ...
              <span
                class="transition-300 text-base"
                :class="{ 'rotate-180': languageDropdown }"
              >
                <svg
          ...
          <DropdownMenuContent align="end" class="w-[180px] z-[999]">
            <DropdownMenuItem
              class="flex gap-2 py-1.5 px-2 cursor-pointer transition-300 hover:bg-white-100"
        ...
        <a href="" class="max-lg:hidden">
          <Button
            class="bg-white/[0.16] border-white/[0.08] text-white text-base"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
